...
description: 進一步了解 AEM 6.2 Cumulative Fix Pack 發行說明。
...
透過附加元件套件和發行版本隨附的其他修補安裝程式來提供 AEM Forms 修正。如需詳細資訊，請參閱 [AEM Forms 發行版本](aem-forms-releases.md)。

### Forms 附加元件套件 {#forms-add-on-package}

>[!NOTE]
>
>針對 AEM Forms 客戶，必須先安裝任何 AEM Service Pack、Cumulative Fix Pack 或 Feature Pack，才能安裝 AEM Forms 附加元件套件。

>[!NOTE]
>
>AEM Forms 附加元件套件用於協助讓表單功能與 AEM Service Pack 和 Cumulative Fix Pack 一致。因此，必須先安裝任何 AEM Service Pack、Cumulative Fix Pack 或 Feature Pack，才能安裝 AEM Forms 附加元件套件。
...
* 以 PDF 預覽信函時，置於主要頁面的文字欄位沒有遵循從資料索引標籤輸入的值或資料連結指定的值。NPR-29239：CQ-4266856 的 Hotfix。
...
* 如果缺少「編輯 ACL」權限，您無法為 DAM 資料夾重新命名。NPR-27555：CQ-104652 的 Hotfix
...
* 在 AEM 6.2 SP1 執行個體上安裝 CFP 18 後，網頁控制台元件詳細資訊會因 500 錯誤而失敗。NPR-28350：CQ-4261095 的 Hotfix
...
* 封裝管理員活動記錄應擷取至獨立的記錄檔中。NPR-27323：Granite-14866 的 Hotfix
...
* 下載功能會透過資產下載 servlet 來使用 AEM Assets，讓匿名使用者下載所有資產。NPR-27054、CQ-4254732 的 Hotfix
...
* RTE RTF 編輯器工具列覆蓋選取清單。NPR-25165：CQ-4248948 的 Hotfix
...
* (全方位搜尋)：「Title」屬性容易遭受網站中的跨網站指令碼 (XSS) 攻擊。NPR-24994：Granite-19933 的 Hotfix
...
* 變數可填入為「清單」資料類型，子類型為字串，但會發生「無法強制轉型物件」錯誤。NPR-26194：CQ-4252287 的 Hotfix
...
* (傳統 UI) 在對話框中進行製作後，完整的標記會顯示為空白。NPR-24688、CQ-4246407 的 Hotfix
...
* 由於管理搜尋功能中有很多請求，出現高 CPU 使用量問題。NPR-23588：Granite-21286 的 Hotfix
...
* 透過預設預覽來預覽信函時，版面片段對應沒有顯示，而按一下預覽時，同樣的版面片段對應則會正確顯示。NPR-22993：CQ-4237745 的 Hotfix
...
* EmbedXMP 資料一律對 TIFF、Pyramid 產生程序設為「作用中」。NPR-22776：CQ-4234498 的 Hotfix
...
* (Firefox) 所選人物誌的核取方塊不正確。NPR-23345
...
* 搭配連字號查詢使用全方位搜尋時會傳回伺服器錯誤。NPR-22999：Granite-19674 的 Hotfix
* 日期挑選器不支援手動設定隱藏欄位所設定的外部類型提示。變更類型提示會出現對話錯誤。NPR-23333：Granite-21194 的 Hotfix

### WCM - Foundation 元件 {#wcm-foundation-components-2}

* 為提高安全性，CAPTCHA 元件現已棄用。如果有使用 CAPTCHA 元件，安裝 6.2 SP2-CFP15 或更高版本後，會顯示訊息「CAPTCHA 元件已棄用，不應使用」。為提高安全性，AEM 元件可供自訂以包含 reCAPTCHA NPR-22151：CQ-4220052 的 Hotfix
...
* 使用快速工具列動作開啟資產的屬性時，單引號字元會使大量檢視中的中繼資料無法更新，且 UI 會損壞。NPR-22317、NPR-22353：CQ-4236990、CQ-4236469 的 Hotfix
...
* (Digital Publishing Suite) 對開本必須要有發佈日期和封面日期欄位，才能上傳至 DPS。NPR-22484

### Commerce {#commerce}

* Commerce 建立目錄精靈中有多個跨網站指令碼 (XSS) 漏洞。NPR-22344：CQ-4237017 的 Hotfix
...
* 推出已在主要版本中移動的結構會導致錯誤 `cq:moveTarget`。NPR-22373：CQ-4232536 的 Hotfix
...
透過附加元件套件和發行版本隨附的其他修補安裝程式來提供 AEM Forms 修正。如需詳細資訊，請參閱 AEM Forms 發行版本。
...
* (HTML 工作區) 當使用者認領任務時，僅該特定使用者的佇列計數會重新整理。也就是說，除非重新整理頁面。NPR-19778：CQ-4233665 的 Hotfix
...
* 時間扭曲會傳回「RecursionTooDeepException」錯誤並出現亂碼頁面，且即使變更日期，也不會建立新版本。NPR-21707：CQ-4199536 的 Hotfix
...
* 使用 Sling Distribution 進行使用者同步時，無法同步使用者。NPR-21911：Granite-20404 的 Hotfix
...
* 全方位搜尋中有過多字元可能會造成 AEM 伺服器當機。NPR-21083：CQ-4223602 的 Hotfix
...
* 升級至 AEM 6.3 後，找不到參考標記實作為指令碼的自訂標記 lib。NPR-20149：Granite-18433 的 Hotfix
...
* 在 RTE 中連結使用了連結外掛程式的已編碼資產檔案時，會傳回空白連結。NPR-20626、NPR-21059：CQ-4223011 的 Hotfix
...
* 請求在封裝管理員中新增驗證選項，以偵測是否對特定套件變更了任何 ACL 權限。CQ-4229196 的 Hotfix
...
* 當叫用服務 OMPFSubmission/PDFA/PDFtoPDFA 時，會擲回名稱為 `java.lang.IllegalArgumentException message:No enum constant com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailureReason.SIGNED_IN_FUTURE` 的例外狀況。此例外狀況會導致在伺服器重新啟動之前無法完成短期簽名驗證處理程序。NPR-20792
...
* 如果伺服器在 org.apache.sling.i18n 2.5.6 中重新啟動，國際化字典會正常運作。NPR-20525：Granite 的 Hotfix- 19490
* 沒有根據具有預設執行緒傾印收集器設定 (預設 AEM 啟動) 的排程器週期產生執行緒傾印。NPR-20288：Hotfix for GRANITE-19488/GRANITE-12741 CQ-90647。
...
* 使用匿名工作階段存取時，ClientLibraryProxyServlet 不會在已發佈執行個體上對用戶端程式庫提出 Proxy 請求，且會擲回 HTTP 404 找不到錯誤。NPR-20195：Granite-14409 的 Hotfix
...
* 從同一頁面建立產品時，不會出現確認或重新導向快顯訊息。NPR-20257：CQ-4223414 的 Hotfix
...
* 即使從多影像就地編輯器元件中移除影像後，影像地圖屬性仍然存在。NPR-20142：CQ-4222982 的 Hotfix
...
* 「CAC/共同驗證憑證選取對話框」應將採用增強金鑰使用方法 (EKU) 的憑證顯示為客戶端驗證或智慧卡登入。NPR-20708
...
* 修正 Context Hub 的獨立持久性快取。
* 精確計算資產維度。
* 將資產發佈至 Brand Portal 時最佳化 AEM 效能。
* 對畫布節點中 `Resourcetype` 值的修正。
* 針對文件片段內容啟用區分大小寫和特殊字元搜尋功能。
* 增強調適型表單，可在 Safari 中附加 PDF 作為附件。此體驗會提供新的 Dynamic Media，可連接至新的 Dynamic Media 發佈基礎架構，以達到更快速、擴展性更高的複寫。

### 資產 {#assets-10}

* AEM Assets 無法擷取 InDesign 資產的子資產參考，包括資產的重複連結。NPR-19006：CQ-4204186 的 Hotfix
* 排序選項無法用於「Commerce」底下集合內的資產。NPR-19508：CQ-4213622 的 Hotfix
...
* 使用者嘗試發佈 `/unpublish/create` 其他使用者鎖定的頁面版本時，AEM 會顯示非預期的行為。NPR-19249；CQ-4215298 和 CQ-4203856 的 Hotfix
...
* 增強調適型表單，可在 Safari 中附加 PDF 作為附件。為支援現有表格的相同功能，需要變更附件 Widget 和「支援的檔案類型」中的設定，以更新值 application/pdf 而非 .pdf。NPR-19623
...
* 在 Safari 瀏覽器上對智慧型集合套用全方位搜尋篩選器時出現問題。NPR-19511
...
* 對話框中有多個 RTE 元件會發生問題。NPR-19311：NPR-19587
...
* 由於 AEM 文章控制台中的完整尺寸文章，縮圖轉譯程序變得緩慢。NPR-19086：CQ-4217148
* 如果使用者擁有多個區域的存取權，透過 Campaign 對優惠方案進行個人化設定時，自動折疊功能會出現錯誤行為。NPR-19290：CQ-4218029 的 Hotfix
* 編輯並儲存目標模組超過一次時，不會在目標定位模式中顯示目標定位對話框。NPR-19144：CQ-4216708 的 Hotfix
...
* 在 AEM 6.2 上套用 CFP6 後，無法編輯調適型表單的 XML 結構描述屬性。CQ-4219684 的 Hotfix
...
* 使用 min:gcc;obfuscate=true 啟用縮製時，Site admin 和 Misc admin 都無法運作。NPR-18593；CQ-4209220 的 Hotfix
...
>[!NOTE]
>
>如果您受到以下兩個問題影響，請勿安裝此 CFP 附加元件套件：
>
...
* 在引導根面板的初始化程式事件時，變更按鈕標題的指令碼無法運作。NPR-18476
...
* Adobe CQ DAM 到期通知工作不會停用過期的資產。NPR-17666；CQ-107766 的 Hotfix
...
* 觸控式 UI 控制項對具有必要權限的使用者是隱藏的。NPR-17945；CQ-4211231 的 Hotfix
* 標記選擇器欄位上遺漏日文標記。NPR-17768；CQ-4210456 的 Hotfix
* FastQuerySize 已啟用時，getsize() 查詢會傳回不正確的結果。NPR-18018
* 無法存取待命執行個體上的網頁控制台。NPR-17861；Granite-14582 的 Hotfix
...
* 缺少核取方塊，因此可以還原繼承。NPR-17753；CQ-4210139 的 Hotfix 請求
...
* 在翻譯專案完成之前，不需要再次為翻譯專案提交資產頁面。NPR-18249；CQ-4209908 的 Hotfix
...
* 安裝附加元件套件時記錄中出現未預期的錯誤。NPR-18295
...
* 使用傳統和觸控式 UI 來移動頁面時，「移動」對話框不會顯示超過 150 個參考，導致使用者無法更新這些參考和重新發佈頁面。此問題已藉由為傳統 UI 引入「maxRefNo」屬性而解決，這個屬性可在網站管理員節點「`'/libs/wcm/core/content/siteadmin'`」上設定。此屬性指定了進行大量移動作業之前顯示的最大參考數 (預設值 150)。如果頁面有許多參考數，則不會顯示在 movePage 對話框中。此設定也適用於 dam admin 和 misc admin，方法是分別在以下節點上套用設定：`'/libs/wcm/core/content/damadmin'` 和 `'/libs/wcm/core/content/miscadmin'`。NPR-17222；CQ-85878 的 Hotfix
...
* 使用 WCM-頁面編輯器元件時，在作者實例的觸控式 UI 編輯模式下，頁面末端存在大量空格。NPR-17510；CQ-4205441 的 Hotfix
...
* 在針對 AB 測試設定的調適型表單中，按一下「開始 AB Testing」時不會開始測試，且會擲回瀏覽器控制台錯誤。NPR-17838
...
* 使用 AEM forms 6.2 輸出服務將特定表單與資料 XML 合併需要花費 20 倍的時間。此問題現已在 Windows 和 Linux® 環境中修正。NPR-17501
...
* 無法藉由在 `/apps` 中覆蓋來將欄新增或自訂至&#x200B;**資產全方位搜尋**&#x200B;結果。NPR-16737：CQ-4206785 的 Hotfix
...
* `ContextHub` 中的檔案不會在作者實例上最小化。NPR-17022：CQ-79456 的 Hotfix
...
* 在 WCM Launches 中，從觸控式 UI 的頁面屬性中起始動作時，與啟動相關聯的頁面無法解除發佈。NPR-16560：CQ-4204681 的 Hotfix
...
* 使用 Firefox 或 Google Chrome 瀏覽器導覽至「**[!UICONTROL 全域導覽]**」窗格時，會提供不同的使用者體驗。Firefox 瀏覽器顯示「**[!UICONTROL 工具]**」選單。Google Chrome 瀏覽器顯示「**[!UICONTROL 導覽]**」選單。NPR-16770；CQ-4200456 的 Hotfix

### 行銷活動 {#campaign}

* 測試 AEM 行銷活動範本和修改種子位址以包含「其他資料」時，Adobe Campaign 下拉式清單會在觸控式 UI Context Hub 中消失。NPR-16771：CQ-105748 的 Hotfix

### Mobile On-Demand {#mobile-on-demand-3}

* 從 AEM Author 預檢發佈內容時，若「預檢」動作耗時超過 5 秒，會導致「AEMM - AEM PECS 整合」Splunk 控制面板上出現異常尖峰，且出現大量狀態請求。NPR-16908：CQ-4207055 的 Hotfix
...
* 在調適型表單編輯器中，head.jsp 中的「Target 設定」註解應以新的 Context Hub 陳述式取代。NPR-17173
* 在調適型表單規則編輯器中，「**[!UICONTROL 選擇項目]**」會將事件顯示為「null」。NPR-17139
* 使用下一頁箭頭 (>) 往下一頁導覽時，已提交的表單會重新提交。NPR-17080
* 透過 AJAX 提交調適型表單時，若發生錯誤，絕不會叫用「error」回呼函數。NPR-17034
...
* 支援在「**[!UICONTROL 預設預填服務設定]**」中使用 `dataRef=C:/Users/` 類型的規則運算式。此類型適用於調適型表單的資料預填。NPR-16425
...
* 規則編輯器的結構「**[!UICONTROL 儲存表單]**」無法運作。NPR-16905
...
* 修正使用目標同步對象、發佈促銷活動，以及在觸控式 UI 中選取目標量度時的問題
* 解決翻譯工作的同步問題
* 為 Forms 預填服務提供增強的安全性
* 改善條碼式表單服務中的表單入口網站草稿和提交元件
* 包含檔案附件 Widget 或延遲載入片段的調適型表單可用性改善。
...
* 封裝管理程式中的驗證選項，可偵測 `/apps` 底下覆蓋的檔案 (JSP 或 JavaScript 檔案) 與 `/libs` 底下 Hotfix 中所包含的檔案之間的任何衝突。接著受影響的覆蓋便可重訂基底以包含 `/libs` 底下檔案的變更。NPR-16216：CQ-81729 的 Hotfix
...
* 從 AEM 6.0 升級至 AEM 6.2 後，傳統 UI 會因為大量查詢而在搜尋標記時出現效能緩慢情形。若要解決此問題，可以依照[在標記控制台 (傳統 UI) 中停用複寫狀態](#disable-replication-status-in-tagging-console-classic-ui-npr)底下提及的步驟操作。NPR-15842：CQ-4201748 的 Hotfix。
...
* 在「觸控式 UI 網站」畫面中，嘗試將頁面貼入沒有子頁面的頁面中時，會因為「貼上」按鈕無法使用而無法運作。NPR-15894：CQ-4201696 的 Hotfix。
...
* 在內容階層下建立標題包含特殊字元的資料夾時，將顯示這些字元的編碼版本。NPR-15935：CQ-4202987 的 Hotfix
...
* 在 AEM 中建立 Target A/B 測試活動時，對象不會同步至 Target 且會顯示「無對象」。NPR-16229：CQ-4204210 的 Hotfix
* 安裝 SP1+NPR-11577 v1.2 後，在觸控式 UI 中進行目標定位期間，為目標量度選擇「使用 Analytics 量度」時，量度的下拉式清單不會載入。NPR-16129：CQ-4204316 的 Hotfix
* 使用目標定位時，發佈促銷活動不會自動發佈整個樹狀結構，包括品牌和主要版本。NPR-15855：CQ-94630 的 Hotfix

### 轉換 {#translation-8}

* 同步翻譯工作現在會自動觸發，現在無需點擊翻譯專案即可進行 AEM 輪詢。NPR-15163：CQ-90856 的 Hotfix
...
* 針對具有檔案附件 Widget 的調適型表單，預覽附加檔案後，表單的某些部分會被停用。NPR：16611

* 對於允許多個附件的檔案附件 Widget，在已有附件的 Widget 上提交帶有附件的新表單執行個體時會顯示錯誤代碼。開啟新增的附件而不是實際內容時會發生此錯誤。NPR-16258
* 保護表單預填服務不會因 `file://`、`http://` 和 `ftp://` 等通訊協定而遭到未經授權的存取。請參閱「[使用 Configuration Manager 設定預填服務](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)」。NPR-15414

* 請求在驗證步驟中以 PDF 格式 (而非 HTML) 來轉譯調適型表單，並將所有附件附加至 PDF，以便讓列印成品顯示完整的表單。NPR-9011

**通信管理**

* 在預覽或編輯模式下處理信函中的文字片段時，如果將文字轉換為清單，便會中斷整個信函功能。並且會產生 JavaScript 錯誤。NPR-16460
...
* 使用 Internet Explorer 瀏覽器預覽有可捲動內容的信件時，瀏覽器的捲動軸會與信件的捲動軸重疊。因此，右側的片段無法查看整個內容。NPR-16068
* 使用 Google Chrome 瀏覽器建立或編輯文字文件片段時，顏色選取下拉式清單會自動彈出，而且無法移除。使用者必須選取清單作為資料項目類型才能編輯片段。NPR-16067
...
* 保護表單預填服務不會因「file://」、「http://,」和「ftp://」等通訊協定而遭到未經授權的存取。如需詳細資訊，請參閱「[使用 Configuration Manager 設定預填服務](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)」。NPR-15434
...
[取得檔案](assets/do-not-localize/osgi_bundle_list_for_aem-6.2sp1-cfp3.txt)。Cumulative Fix Pack 2 的關鍵重點為：

* AEM 平台的穩定性修正和效能改善，包括 Sling 架構和作業在內
* 透過針對資產存取、移動、搜尋、上傳和發佈的多項修正改善資產管理
* 透過內容片段、錨點外掛程式、投影片放映和 Context Hub 元件的修正，改善 Sites 的製作和管理功能
* 觸控式 UI 中的多項修正，包括文字編輯器、全方位搜尋和變量建立程序
...
* 導覽至頁面元件時，如果值的標記具有 `tagBasePath` 屬性，傳統 UI 對話框中就不會擷取該值。NPR-15543：CQ-4199950 的 Hotfix
...
* 使用者在 CRXDE 中選取一個節點 (例如 `/content/dam/`)。然後按一下「存取控制」標籤，確保存取控制清單存在。現在，拖放某些元素會移動元素，而不是選取的元素。NPR-15696 GRANITE-16300 的 Hotfix
* 嘗試進行模擬時若從下拉式清單中選取使用者，會導致整個使用者快顯視窗消失。NPR-15774：CQ-4201738/GRANITE-11895 的 Hotfix
* 在全方位搜尋中，無法使用自動填入建議的標記來搜尋。NPR-15088：GRANITE-14426 的 Hotfix。注意：此修正需要 Oak CFP 1.4.11 或更新版本。
...
* 在內容片段編輯器 UI 中，建立許多內容片段變化版本時，側邊面板不會顯示用於導覽所有變化版本的捲軸。NPR-15445：CQ-4199444 的 Hotfix
...
* 針對使用 Live Copy 的網站，在網站 Admin Console 的「欄」窗格中選取「Live Copy」核取方塊時，無法正確顯示 Live Copy 狀態，只會顯示 HTML 標記。NPR-15108：CQ-97086 的 Hotfix
...
* 在全方位搜尋中搜尋資產後，選取資產並按一下「**檢視屬性**」以嘗試編輯屬性，然後按「**儲存**」按鈕時，會將使用者重新導向至空白頁面。NPR-15900：CQ-4202372 的 Hotfix
* Assets 使用者介面不會回應事件。選取資產並按一下「發佈」或「轉譯」時不會產生任何活動。NPR-15828：CQ-4202247 的 Hotfix
* 從卡片檢視發佈資產時，不會更新卡片以反映已發佈狀態。反而是頁面會被重新整理。NPR-15826：CQ-102732 的 Hotfix
...
* 使用數位資產管理器時，如果頁面與一組影像相關聯，將影像移至新資料夾時會中斷頁面關聯。因此，關聯的頁面會遺失部分影像。NPR-15538：CQ-111479 的 Hotfix
...
* 在&#x200B;**[!UICONTROL 觸控式 UI]** 中，使用 Internet Chrome 瀏覽器 56.0.2924.87 版時，針對現在有 type=&#39;datetime’ 的日期欄位，會顯示儲存的日期。NPR-15383：Granite-16481 的 Hotfix
...
* 全方位搜尋不允許使用 `/apps` 的客戶新增欄資料來源，因為它假設位置設定列在 */libs/granite/omnisearch/content/metadata/* 底下。NPR-13188 GRANITE-16479 的 Hotfix
...
* 使用「Campaign - 目標定位」和「`MAC` - 測試和 Target 整合」元件時，取消發佈活動時不會更新主要 UI 中的活動狀態。NPR-15401：CQ-4199839 的 Hotfix
...
* 即使預填 XML 的 XML 結構描述式調適型表單中沒有 `afData/afBoundData` 包裝函式，`afData/afBoundData` 包裝函式仍存在於已提交的 XML 中。NPR-15541
...
* 請求將 Jackrabbit 檔案保存庫升級至 3.1.30 以解決多個問題。NPR-13454
...
* 建立多個 RTF 欄位 (例如連結) 時，Target 下拉式清單無法在觸控式 UI 對話框中運作。NPR-13911
...
* 從編輯器提交 PageProperties 對話框時，會重寫 LiveCopy 索引標籤中未變更的資料。NPR-14693
...
* 無法正確顯示具有「條件式重複」運算式的資產。使用者預覽 `*CDN3835RLCEN*` 信函範本時，沒有顯示位於內文目標區域中的資產。對於允許多個附件的檔案附件 Widget，在已有附件的 Widget 上提交帶有附件的新表單執行個體時會顯示錯誤代碼。NPR-14844
...
* 有複寫權限的使用者現在可以刪除資產資料夾。NPR-14321：CQ-88271 的 Hotfix
* 嘗試在管道編輯器中編輯視訊的視訊設定檔時，設計對話框不會開啟，且會在錯誤記錄中記下「Null 指標例外狀況」。NPR-14144：CQ-81101 的 Hotfix
...
* AMS AEM Assets 作者實例上，從不同網路進行存取的使用者若沒有資料夾刪除權限，便無法使用區塊上傳功能來上傳資產。NPR-12768：CQ-82715 的 Hotfix
...
* 請求為 AEM Mobile-on-demand 服務實作全方位搜尋功能。對任何文章、集合或橫幅進行關鍵字搜尋都不會傳回任何相符項目。NPR-14093：CQ-101394 的 Hotfix
...
* 在工作流程控制台的「封存」索引標籤中，按一下裝載連結時，會傳回一個回應代碼為「404」的錯誤。NPR-14993：CQ-4194977 的 Hotfix
...
* 使用 SAML 驗證處理常式設定的「saml_request_path」Cookie 會顯示額外 `?` 字元此外，將 SAML 回應發佈回 AEM 時，AEM「saml_request_path」Cookie 會因為字元已編碼而傳回無效值。NPR-13517：GRANITE-11722 和 GRANITE-14414 的主動式 Hotfix
...
* 如果您為選項按鈕設定預設值並在儲存或提交期間選取不同值，則預先填入不會顯示選取的值。NPR-15304
...
`Mobile Forms` (Forms 附加元件套件)：

* 若調適型表單是從 XDP 檔案建立，協助工具的 Tab 鍵瀏覽順序不會遵循設定模式。NPR-12562

`Adaptive forms` (Forms 附加元件套件)：
...
>[!NOTE]
>
>針對 AEM Forms 客戶，必須先安裝任何 AEM Service Pack、Cumulative Service Pack 或 Feature Pack，才能安裝 AEM Forms 附加元件套件。
...
* AEM 6.2 Service Pack 1 是 CFP 的必備條件。如需安裝指示，請參閱 [AEM 6.2 Service Pack 1](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) 發行說明。
...
* 對於使用 (RDBMK 或 MongoDB) 的叢集部署，CFP 套件可安裝在使用封裝管理員的任何作者實例上。
...
1. 開啟[封裝管理員](http://localhost:4502/crx/packmgr/index.jsp)，然後按一下&#x200B;**[!UICONTROL 「上傳套件」]**&#x200B;即可上傳套件。
...
* [從封裝管理員使用 HTTP API](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) (請務必使用 `cmd=install&recursive=true`) 以安裝巢狀套件。
...
1. 在 OSGi 控制台 (使用網頁控制台：/system/console/bundles) 中，所有 OSGI 套件組合均為「作用中」或「片段」。
...
1. 下載適用於您作業系統的 [AEM Forms 發行版本](aem-forms-releases.md)所列出的對應 Forms 附加元件套件。
...
CFP12.1 中包含 NPR-21355。若要安裝 CFP12.1 或更新版本，請執行下列程序，在 JBoss® 應用程式伺服器上設定 NPR-21355。若您是在 Oracle WebLogic 或 IBM® WebSpehere 應用程式伺服器上所執行的 AEM Forms Server 上安裝 CFP12.1，則無需進行額外設定。

1. 備份、刪除和建立 module.xml 檔案。檔案的預設位置為 [AEM_Forms_Installation_directory]/jboss/modules/system/layers/base/com/adobe/livecycle/main/
...
1. 在 [AEM_Forms_Installation_directory]`/jboss/modules/system/layers/base/com/adobe/livecycle/main/` 處建立 `jsafeFIPS.jar`、`jsafeJCEFIPS.jar` 和 `certjFIPS.jar` 檔案的備份，並從前面提到的目錄中刪除這些檔案。
...
使用者認領任務時，不會重新整理其他使用者的共用佇列計數。因此，Adobe 推出了一項新屬性。請依照下列步驟操作，在您的 AEM 執行個體上設定此屬性：

1. 前往「管理員 UI -> 服務 -> 工作區 -> 全域管理」。
1. 匯出全域設定。
1. 在下載的 XML 檔案中，新增標記 `<client_tasksPollingInterval>10</client_tasksPollingInterval>`。10 是範例值 (以秒為單位)。您可以視情況修改此值。
1. 儲存檔案。
...
* 開啟網頁控制台元件詳細資訊頁面時，會收到 HTTP 500 內部伺服器錯誤。
...
* 在 AEM 6.2 SP1 (包含智慧標記 Feature Pack) 上安裝 CFPx 時，先前新增的智慧標記資產的工作流程步驟會從 DAM 更新資產工作流程中刪除。

## Uber Jar {#uber-jar}

適用於 6.2 SP1-CFP20 的 Uber Jar 可於 Adobe Public Maven 存放庫取得。
...
>[!MORELIKETHIS]
>
>* [AEM 6.2 Hotfix 頁面](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)
>* [AEM 6.2 SP1 發行說明](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [AEM 6.2 發行說明](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [AEM 產品頁面](https://business.adobe.com/products/experience-manager/adobe-experience-manager.html)
>* [AEM 6.2 檔案](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [Adobe 優先產品更新](https://experienceleague.adobe.com/zh-hant/docs/release-notes/experience-cloud/current)
