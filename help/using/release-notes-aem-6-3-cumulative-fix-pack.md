...
| **必備條件** | [AEM 6.3 Service Pack 3 (6.3.3.0)](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) |
| **正式發行** | 2020 年 3 月 5 日 |
...
### Campaign 目標市場選擇 {#campaign-targeting}

* 在作者實例中變更然後啟動的元件內容，在重新啟動前不會在發布實例中顯示&#x200B;**com.day.cq.personalization.impl.TargetedContentManagerImpl** (NPR-32489 和 NPR-32232)。
...
* 即便使用預設篩選器後，資產仍從在 Dynamic Media - Scene7 部署中執行的製作執行個體複製到發布實例 (NPR-30856)。
...
* 使用者著重在左側邊欄欄位，並會使用鍵盤快捷鍵來貼上內容。這樣做時，系統會貼上頁面編輯器的剪貼簿內容，而不是貼上從左側邊欄欄位複製的內容 (NPR-31173)。
...
* 在 AEM Author 環境中不會填入分析報告。相反，會顯示一個空白頁面 (NPR-30905)。
...
* 當您使用智慧影像時，無法將體驗片段匯出至目標。CQ-4269606 的 Hotfix
...
* 檔案 `coral.js` 包含易受攻擊版本的函式庫 `handlebars.js`。NPR-26973：CQ-4255377 的 Hotfix
* 使用含有巢狀配置容器和文字元件的 Target 元件，會導致在編輯文字或按一下容器時擲回 JavaScript 錯誤「無法讀取 null 的 currentPos 屬性」。NPR-29077：CQ-4246594 的 Hotfix
* (觸控式 UI) 無法將標記大量更新至已經使用不同標記加以標記的頁面。NPR-28729：CQ-4262922 的 Hotfix
* 在卡片檢視中開啟變化造成了 500 錯誤。NPR-28611：CQ-4263571 的 Hotfix
* 推出已在主要版本中移動的結構會導致錯誤 `cq:moveTarget`。NPR-28968：CQ-4265280 的 Hotfix

### 整合 {#integration}

* (雲端服務設定) 根層級顯示的「繼承自」核取方塊應移除。NPR-28771：CQ-4259676 的 Hotfix
* com.day.cq.personalization.impl.TeaserResourceEventHandler 進入無限迴圈，並造成發布實例上的節點更新。NPR-28561：CQ-4263096 的 Hotfix
...
* 按一下標記會移除全域 mouse-up 事件，且對話框會凍結在可拖移模式下。NPR-28641：CUI-7294 的 Hotfix
...
* 一個共用的佇列回應任務會在 HTML5 工作區中開啟一個 Flash 元素。NPR-29161：CQ-4266498 的 Hotfix
* 如果按鈕包含 Umlaut 字元，就無法從工作區提交。NPR-29014：CQ-4263172 的 Hotfix

#### Forms - 文件安全性 {#forms-document-security}

* 啟用可選取原則集檢視頁面上每頁項目的選項。NPR-29243：CQ-4268567 和 CQ-4265132 的 Hotfix
...
* 在發布實例上將 rep:cache 新增至 AEM Communities 使用者同步接聽程式的可忽略節點中。
...
* (反向移植) 在發布實例上將 rep:cache 新增至 AEM Communities 使用者同步接聽程式的可忽略節點中。NPR-27841：CQ-4247234 的 Hotfix
* 在搜尋方塊中，特殊字元會有逸出字元 (\) 首碼。NPR-27839：CQ-4259757 的 Hotfix
...
* 內容框架在 editor.js 中載入後沒有清除清單 `loaderPromises` NPR-27768：CQ-4205337 的 Hotfix
...
* 由於已顯示動作，LiveCopy 繼承取消現在可在目標容器上正常運作。NPR-28129：CQ-4259813 的 Hotfix

#### 複寫 {#replication-1}

* DispatcherFlushRules 已中斷 6.3.3.1 中的複寫 NPR-28150：CQ-4261401 的 Hotfix

#### Campaign - 目標市場選擇 {#campaign-targeting-1}
...
* 無法在 HTML5 工作區上轉譯 PDF Forms。NPR-28059：CQ-4260373 的 Hotfix
...
* 勾選核取方塊項目時，新增或移除下拉式清單中的項目沒有更新。NPR-28224：CQ-4252834 的 Hotfix
...
* 在「資料夾中繼資料結構」中新增對「規則索引標籤」和其在「資產資料夾」中強制執行的支援。
* 啟用分頁支援以分組發佈時列出的頁面。
* 啟用可設為任何數字的 unreadCount 通知。預設值為 20。
* 外部連結檢查程式的修正。
...
* 透過連結分享電子郵件下載會傳回一個空白/損毀 zip 檔案。NPR-27997：CQ-4259686 的 Hotfix
...
* AEM Forms 出現 XML 插入漏洞。NPR-27863：CQ-4257315 的 Hotfix
...
* Form Builder 升級為最新 Handlebars 版本。
...
* 透過解封存器步驟建立的封存檔中若包含名稱中有 % 的資料夾，則解壓縮該封存檔後無法開啟。NPR-26275：CQ-4251482 的 Hotfix
* 一個單引號字元導致中繼資料無法更新。NPR-26808：CQ-4254305 的 Hotfix
...
* 在編輯對話框中，原則不會控制 RTF 編輯器功能。NPR-27059、NPR-26750：CQ-4241130 的 Hotfix
...
* 結構化的內容片段模型標記欄位不會反映標記移動操作。NPR-26801：CQ-4251805 的 Hotfix
...
* 發佈含有新資產的頁面時，未經通知便停用了來自發布實例的子頁面。NPR-24869：CQ-4247832 的 Hotfix
* 複寫使用過期的 OAauth 代號。NPR-25984：Granite-22388 的 Hotfix

#### 複寫 {#replication-2}

* Http 傳輸可能洩漏開放工作階段。Granite-17434 的 Hotfix
* 在存取存取語彙基元節點時，透過多個複寫代理程式同步存取會導致記錄出現異常。NPR-26964：GRANITE-23276、Granite-23155 的 Hotfix

#### ContextHub {#contexthub}

* 檔案 `coral.js` 包含易受攻擊版本的函式庫 `handlebars.js`。CQ-4255377 的 Hotfix
...
* 在論壇元件上搜尋時出現問題。NPR-26577：CQ-4251303 的 Hotfix
...
* 無法建立或編輯文件安全性的浮水印設定。NPR-26267、NPR-26129：CQ-4250234 的 Hotfix
...
* 編輯和儲存可編輯模組，然後開啟或關閉 PDF 預覽，可持續顯示信件的 PDF 和 HTML 預覽。兩個預覽在同一視窗中仍然有效。NPR-26770：CQ-4253217 的 Hotfix
...
* 與起始點相關聯的非預填調適型表單或表單集開啟時，系統會擲回一個 Null 指標例外狀況。CQ-4256124 的 Hotfix
* 無法使用全域設定傳送電子郵件。NPR-26163：CQ-111110 的 Hotfix
* 套用 axis.jar 檔案後無法開啟工作區。NPR-26131：CQ-4251126 的 Hotfix
* 重新整理按鈕無法將最新資料從伺服器同步至調適型表單。CQ-4255670 的 Hotfix
* 從管理員 UI 設定搜尋範本，然後使用相同的 UI 在 AEM Forms 工作區中搜尋時，系統便會擲回例外狀況。CQ-4255649 的 Hotfix
...
* 從 LiveCycle 升級為 AEM 6.2 Forms 時，記錄中出現 org.jgroups.Message 錯誤。NPR-26795：CQ-4220415 的 Hotfix
...
* 下載大小與顯示的資訊不符。NPR- 23418：CQ-4242774 的 Hotfix
...
* 頁面屬性中的縮圖索引標籤在延伸對話框中無法運作。NPR-22844：CQ-4241474 的 Hotfix
...
* 在另一個工作階段中的另一位使用者可修改使用者在第一個工作階段中鎖定的頁面。NPR-23057；CQ-4199017 的 Hotfix
...
* 當重新啟動套件組合時，國際化翻譯現在會更新。NPR-23193：Sling-7190 的 Hotfix
...
* (Target) 使用如 XML 的 API 類型時，促銷活動沒有啟用。NPR-23199：CQ-4240936 的 Hotfix****
...
* 社群搜尋因幾個字而失敗。NPR-23256：CQ-4240717 的 Hotfix
...
* 透過 Adobe Sign 支援調適型表單整合中的多重分區。NPR-23383
...
* 為透過 Designer 和 Output Service 產生的 XML Forms 新增 PDF/UA 支援。NPR-23132
...
* 嘗試將資產資料結構從 `/conf/global` 複製到 `/conf/mytenant` 時，AEM 會擲回 StackOverflow 錯誤。NPR-22489：CQ-4235875 的 Hotfix
...
* 在發布實例關閉的情況下啟用頁面會導致進入綠色狀態而非變為琥珀色。NPR-22927：CQ-4236310 的 Hotfix
* (StyleSystem) 從快顯視窗選擇樣式時螢幕位置跳躍。NPR-23183：CQ-4238867 的 Hotfix
* (管理發佈) 需點按多次才能移至下個月的行事曆。NPR-23508：CQ-4242732 的 Hotfix

### 平台 {#platform-2}

* Sling Exporter servlet 無法正確匯出日文字元。NPR-22153：CQ-4228920 的 Hotfix
* 啟動期間排程器鎖死。NPR-22440：Sling-7004 的 Hotfix
* 無法同步兩個發布實例之間的群組。NPR-21943：Granite-19564 的 Hotfix
...
* 目標引擎 (mbox.js、at.js) 不使用損壞的 URL。同時，使用包含冒號的 URL，在某些部署中可能會失敗。NPR-22366：CQ-4237854 的 Hotfix
...
* 使用英文以外的語言時，AEM 目標市場選擇會失敗。NPR-22917：CQ-4218213 的 Hotfix
...
* 為提高安全性，CAPTCHA 元件現已棄用。如果有使用 CAPTCHA 元件，那麼安裝 6.3.2.1 或更高版本後，訊息「Captcha 元件已廢止且不應該再使用」。為提高安全性，AEM 元件可供自訂以包含 reCAPTCHA NPR-22151：CQ-4220052 的 Hotfix
...
* 當您檢查 ECM 連接器設定頁面的密碼欄位時，密碼會以純文字顯示。NPR-22508：CQ-4236065 的 Hotfix
...
* 規則編輯器的自訂功能正在增加額外的功能；在函數呼叫之後。因此，即使自訂函數傳回 true，這些也無法通過驗證。NPR-22481：CQ-4235499 的 Hotfix
* 顯示驗證訊息下限和上限時，無論選取的日期模式為何，日期選取器元件都不會依循該模式。NPR-22444：CQ-4236269 的 Hotfix
* 提交請求中傳送的日期格式應與日期選取器元件中提供的模式一致。NPR-22384
* Android™ 6.0 Samsung 裝置不遵循為調適型表單文字方塊指定的字元數上限。NPR-22363、NPR-22364：CQ-4235205 的 Hotfix
* (Microsoft® Edge) (IE11) 調適型表單文字欄位元件讓多行欄位的預設值顯示為 `Null` 而非空白。NPR-22284：CQ-69107 的 Hotfix
* 調適型表單中的 SOAP UTF-8 輸入編碼傳回錯誤並出現亂碼頁面。NPR-20105：CQ-4222669 的 Hotfix
...
* (Firefox) 因為文件記錄 (DOR) 中的選項不是屬性頁面中的預先選取項目，因此無法更新調適型表單的 XML 結構屬性。NPR-22298：CQ-4237402 的 Hotfix
* 發佈頁面之後修改的 Forms 不會再發佈一次。NPR-23013：CQ-4236566 的 Hotfix
...
* 瀏覽器會受阻而無法將輸入的憑證儲存至 HTML Forms 中。NPR-21762：CQ-4206543 的 Hotfix
...
* 透過 HSM 以數位方式簽署 PDF 文件時，發生例外狀況。NPR-21154：CQ-4226978 的 Hotfix
...
* 全方位搜尋中有過多字元可能會造成 AEM 伺服器當機。NPR-20434：CQ-4223602 的 Hotfix
...
* AEM Dynamic Media Classic (以前稱為 Scene7) 無法從 AEM 伺服器進行資產同步。當「Cloud Service 設定」點的目標路徑指向根路徑子資料夾時，會發生此問題。CQ-4228265
...
* 在 Adobe Campaign 中實作一套機制，用於從外部目標市場選擇資源擷取內容資源的原則。NPR-20667
...
* Search&amp;Promote Cloud Service 設定忽略了表單擷取方法，導致無法在作者實例上使用。NPR-20447：CQ-4206076 的 Hotfix
...
* 現在有為其他 data-sly-resource 陳述句取消設定 requestAttributes。NPR-20639：4226206 的 Hotfix
...
* 無論對話框中的屬性為何，顏色欄位均設為「永遠必要」。NPR-19702
...
* 即使資產類型不是 XDP，仍會為調適型表單顯示 HTML 設定檔。NPR-20079

#### 後端整合 {#backend-integration-2}

* 無法將切換元件的值設定為 true 或 false。NPR-21111
...
* 針對 dc:title 屬性設為 String `String [] (multifield)` 的影像，在影像卡片上顯示標題的行為變更。
* 以基礎時間元件取代伺服器端的時間轉譯。
* 已啟用透過標記管理員/標記控制台將標記從 AEM 發佈至 Brand Portal 的功能。
* 發佈 JSON API 以使用內容片段。
...
* 設定精靈中的目標市場選擇引擎欄位設為 ContextHub (AEM) 而非 Adobe Target。NPR-19320：CQ-4218465 的 HF
* 建立體驗期間對象區段損毀。NPR-19110
* 編輯並儲存目標模組超過一次時，不會在目標市場選擇模式中顯示目標定位對話框。NPR-19144：CQ-4216708 的 Hotfix
...
* 在製作對話框中，粗體、斜體、底線功能無法用於 RTF 編輯器欄位。NPR-19670：NPR-19718：CQ-4219088 的 Hotfix
...
* 升級至 AEM 6.3 後，找不到參考標記實作為指令碼的自訂標記庫。NPR-19087
...
* 專案編輯器無法將資產複製並貼上至專案資產資料夾中。NPR-19619：CQ-4215321 的 Hotfix
...
* 在 Safari 中調適型表單不允許 PDF 作為附件。NPR-19625
* RestoreGuideState 會覆寫 customcontextproperty 對應。CQ-4222877
* 使用 Cloud Service 設定 Google `reCaptcha` 時，在需要 org.apache.http.proxyconfigurator 設定以進行外部連線的環境中，POST 似乎不會通過 PROXY。NPR-20454
* 安裝時以 XSD 結構為基礎的調適型表單所用的地區設定包含句點 (.) 以外的小數分隔符號，而對數值欄位提交了有問題的 XML 值，進而導致出現錯誤。NPR-20444
...
* 針對非英語地區設定，作為 SOAP 服務輸出的 UTF-8 字元沒有正確複製到調適型表單欄位中。NPR-20238：NPR-20103
...
* 上傳了重複資產時，如果沒有在偵測到重複項目的對話框中選擇選項，則 (刪除/保留) 選項會套用至所有資產。CQ-4201673
...
* PDF Generator 中對 PDF/A-1b 驗證、大型影像檔案轉換為 PDF，以及日文 PDF 文件的修正。
* 通信管理、文件安全性和 Forms Workflow 的可用性修正。
* 新增支援以擷取手寫簽名欄位的稽核事件。
...
* PDF Generator 服務無法將大於 100 MB 的影像轉換為 PDF 文件。CQ-4208628
* 搭配日文 OCR 使用 PDF Generator 服務時，會產生反轉的 PDF。NPR-17602
...
* 改善下列領域：
   ...
   * 內容片段、Sites 以及 RTF 編輯器、規則編輯器和範本編輯器元件
...
>[!NOTE]
>
>Adobe 建議您在嘗試安裝之前，先對每個新安裝套件都執行驗證。安裝之前預先驗證程序會分析並報告所有發現的錯誤，並主動警告使用者出現這些錯誤。
>
>如需驗證選項的說明文件，請前往 [https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)

* AEM 6.3.3.0 是 CFP 的必備條件。參閱[升級說明文件](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)，取得有關將 AEM 安裝升級為 AEM 6.3 的詳細指示。
...
activity.log 是建立在 crx-quickstart/logs 資料夾中
...
>[!NOTE]
>
>若您是安裝 6.3.3.0 或更新版本，請執行以下程序來配置 JBoss® 應用程式伺服器的設定。若您是在 Oracle WebLogic 或 IBM® WebSpehere 應用程式伺服器上所執行的 AEM Forms Server 上安裝 6.3.3.0，則無需進行額外設定。如需更多詳細資訊，請參閱 [AEM 6.3.3.0 發行說明](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)。
...
* 由於管理搜尋功能中有很多請求，出現高 CPU 使用量問題。NPR-24229
...
`b)` 主體：reference-adjustment-service
類型：允許
權限：jcr:read , jcr:modifyProperties
限制：rep:glob=&quot;/&#42;/jcr:content/&#42;&quot;
...
預設值為 1。透過覆寫檔案 constants.js (`/libs/cq/ui/widgets/source/constants.js`) 並取消註解 `CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL`  屬性來覆寫該值。然後，為其指派所需的值 (儲存頁面屬性資料的頁面 `/jcr:content` 下的最大深度)。
...
* 將其值設為 >1000 (目前的預設值)，讓此數大於 `/jcr:content` 子樹狀結構中所設定深度之上的總節點數。

此程序可讓 Sling GET servlet 得以傳回所有需要的節點。

## Uber Jar {#uber-jar}

適用於 6.3.3.8 的 Uber Jar 可於 Adobe Public Maven 存放庫取得。

若要在 Maven 專案中使用 Uber Jar，請參閱[如何使用 Uber Jar](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)，並在您的專案 POM 中加入以下相依性：
...
>[!MORELIKETHIS]
>
>* [AEM 發行版本和更新](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)
>* [AEM 6.3 Hotfix 頁面](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)
>* [AEM 6.3 發行說明](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [AEM 產品頁面](https://business.adobe.com/products/experience-manager/adobe-experience-manager.html)
>* [AEM 6.3 檔案](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
