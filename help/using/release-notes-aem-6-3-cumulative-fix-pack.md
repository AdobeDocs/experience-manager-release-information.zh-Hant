...
* 在製作執行個體中變更元件的內容，然後啟動後，在元件重新啟動之前，不會顯示在Publish執行個體中&#x200B;**com.day.cq.personalization.impl.TargetedContentManagerImpl** (NPR-32489和NPR-32232)。
...
>[!NOTE]
>
>AEM Forms 附加元件套件用於協助讓表單功能與 AEM Service Pack 和 Cumulative Fix Pack 一致。因此，必須先安裝任何AEM Service Pack、Cumulative Fix Pack或Feature Pack，才能安裝AEM Forms附加套件。
...
* 從「僅內容」下拉式選單中選取篩選器選項，然後選取移動選項之前，已選取Assets （在觸控式UI的欄檢視中）也會移動(NPR-30693)。
...
* 使用者轉出頁面時，轉出設定對話方塊顯示重複的Live Copy路徑(NPR-30438)。
...
* 使用者聚焦於左側邊欄欄位，並使用鍵盤快速鍵貼上內容。 這麼做時，它會貼上頁面編輯器剪貼簿的內容，而非從左側欄欄位複製的內容(NPR-31173)。
* 使用者編輯內容片段時，系統會還原該內容片段的已刪除變數(NPR-31272)。
...
* Analytics報表不會填入AEM作者環境中。 相反地，出現一個空白頁面(NPR-30905)。
...
* Dynamic Video的視訊彙總只傳回結果集中的前100個專案。 NPR-30441；CQ-4213561 的 Hotfix
* Adobe 智慧型標記透過 Data Power 連線的問題。NPR-30026：CQ-4269457 的 Hotfix
* 封存檔中的資料夾名稱具有百分比符號(%)，解壓縮後無法使用Assets介面開啟。 NPR-29989：CQ-4270467 的 Hotfix
...
* 產生CSV匯出時，為「清單檢視」新增自訂欄會中斷報表。 NPR-30208：CQ-4273344 的 Hotfix
* /etc/reports/中的OOTB報表無法正常運作，且歷史資料圖表未顯示。 NPR-30016：CQ-4220180 的 Hotfix
* resourceType要求引數的值會複製到以雙引號包封的HTML標籤屬性值中。 NPR-29832：CQ-4255365 的 Hotfix

### 社群 {#communities-1}

* AEM Communities稽核控制檯出現重複內容問題。 NPR-30667：CQ-4276829 的 Hotfix
...
* 對PDF服務的HTML執行負載測試時，發生錯誤而失敗，且檔案型別設定從AEM Forms伺服器移除。 NPR-30111、NPR-30086：CQ-4271495 的 Hotfix
...
* 上傳超過2 GB的Adobe Illustrator Artwork檔案會產生Java™棧積空間錯誤。 NPR-29265：CQ-4226217 的 Hotfix
...
* 搭配多欄位使用時，CoralUI在元件層級儲存fileReferenceParameter而非多欄位層級。 NPR-29535：CQ-4266129 的 Hotfix
* 在 AEM 6.3.3.3 上嘗試比較最新和目前頁面版本時發生問題。NPR-29532：CQ-4269639 的 Hotfix
* 路徑欄位會在根路徑中開啟，無論指定要搜尋的路徑為何。 NPR-29398：CQ-4268897 的 Hotfix
* （體驗片段） FacebookApplication#setUpApp使用已棄用的API，而該API已失效。 NPR-29213：CQ-4266630 的 Hotfix
* 在執行個體上啟用縮制的情況下，將元件新增至WCM頁面時會產生錯誤警報。 NPR-29476：CQ-4266197 的 Hotfix
* 轉出期間沒有從Blueprint傳播空白屬性和多個屬性。 使用藍圖重設 Live Copy 不適用於元件。NPR-29252：CQ-4264928、CQ-4264926、CQ-4267722 的 Hotfix
...
* 使用智慧型影像時，您無法匯出體驗片段至目標。 CQ-4269606 的 Hotfix

### 複寫 {#replication}

* 復寫代理元件疑似為漏洞，向未獲授權的使用者洩漏了敏感資訊。 NPR-29613：Granite-25070 的 Hotfix
...
* 工作流程CommandLineProcess因檔名中有單引號而失敗。 NPR-28805：CQ-4262287 的 Hotfix
* 使用篩選器時，集合頁面和集合頁面之間的值不同。 NPR-28642：CQ-4261405 的 Hotfix
...
* 檔案`coral.js`包含易受攻擊的程式庫`handlebars.js`版本。 NPR-26973：CQ-4255377 的 Hotfix
* 使用含有巢狀配置容器和文字元件的Target元件，會在編輯文字或按一下容器時擲回JavaScript錯誤「無法讀取null的currentPos屬性」。 NPR-29077：CQ-4246594 的 Hotfix
* （觸控式UI）無法將大量標籤更新至已使用不同標籤進行標籤的頁面。 NPR-28729：CQ-4262922 的 Hotfix
* 在卡片檢視中開啟變化時發生500錯誤。 NPR-28611：CQ-4263571 的 Hotfix
* 轉出已在主要位置中移動的結構會導致錯誤的`cq:moveTarget`。 NPR-28968：CQ-4265280 的 Hotfix
...
* 按一下標籤會移除全域滑鼠上移事件，且對話方塊會以可拖曳模式凍結。 NPR-28641：CUI-7294 的 Hotfix
...
* 共用佇列回應任務會在HTML5工作區中開啟Flash元素。 NPR-29161：CQ-4266498 的 Hotfix
* 如果按鈕包含umlaut字元，則無法從Workspace提交。 NPR-29014：CQ-4263172 的 Hotfix

#### Forms - 文件安全性 {#forms-document-security}

* 啟用原則集檢視頁面上選取每頁專案的選項。 NPR-29243：CQ-4268567 和 CQ-4265132 的 Hotfix
...
* 在Designer中將XDP預覽為PDF和將XDP預覽為與HTML相同的XDP時會遇到不同的行為。 NPR-28602：CQ-4260239 的 Hotfix
...
* 為清單和「卡片檢視」按鈕新增Aria標籤。 NPR-27736：CQ-4254027 的 Hotfix
* (反向移植) 在發佈執行個體上將 rep:cache 新增至 AEM Communities 使用者同步接聽程式的可忽略節點中。NPR-27841：CQ-4247234 的 Hotfix
* 在搜尋方塊中，特殊字元帶有逸出字元(\)前置詞。 NPR-27839：CQ-4259757 的 Hotfix
* 在快速搜尋中搜尋`(`、`+`和`?`等字元時發生錯誤。 NPR-28212：CQ-4260969 的 Hotfix
* 無法使用 API 刪除使用者產生的內容中的評論。NPR-28075：CQ-4260534 的 Hotfix
* 有新評論發佈時，發佈到下一頁的評論以黃色醒目提示。 NPR-28148：CQ-4259681 的 Hotfix
* 無法開啟 Outlook 寄件匣和收件匣資料夾中含有附件的訊息。NPR-28559：CQ-4217072 的 Hotfix

#### 網站 {#sites-5}

* 在 AEM 6.3 中執行「版本清除」會造成記錄中出現不斷重複的警告。NPR-27750；CQ-4206870 的 Hotfix
* RTF編輯器全熒幕模式不支援樣式外掛程式。 NPR-27622：CQ-4258674 的 Hotfix
* 內容框架載入editor.js後未清除清單`loaderPromises` NPR-27768：CQ-4205337的Hotfix
* 無法在巢狀Parsys上設定範本原則，除非是在父元件上設定。 NPR-27987：CQ-4246095 的 Hotfix
...
* ResourceResolver沒有解析Target元件的多個網域。 NPR-28265：CQ-107847 的 Hotfix
* 當動作顯示時，LiveCopy繼承取消現在可在目標容器上正常運作。 NPR-28129：CQ-4259813 的 Hotfix
...
* 預設受眾以外的受眾無法使用設定Target元件選項。 CQ-4261370 的 Hotfix
...
* 勾選核取方塊專案時，新增或移除下拉式清單中的專案沒有更新。 NPR-28224：CQ-4252834 的 Hotfix
...
* 在「資料夾中繼資料結構」中新增對「規則標籤」的支援，以及在資產資料夾中強制執行該標籤。
* 啟用分頁支援以分組發佈時列出的頁面。
* 啟用可設為任何數字的 unreadCount 通知。預設值設為20。
* 外部連結檢查程式的修正。

#### Assets {#assets-6}

* 動態下拉式清單不支援階層式下拉式清單。NPR-27044；CQ-4252564 的 Hotfix
* 改善查詢，使其使用ExpiryNotification功能。 NPR-26999：CQ-4251188 的 Hotfix
* 規則從中繼資料結構移轉至資料夾中繼資料結構。NPR-27771：CQ-4257737、CQ-4257735、CQ-4259822 的反向移植
* 資產參考調整無法更新Sling ResourceCollections所屬資產的參考。 NPR-26759：CQ-4252605 的 Hotfix
* 透過連結共用電子郵件下載傳回空白或損毀的zip檔案。 NPR-27997：CQ-4259686 的 Hotfix
* 混合視訊編碼不完整且沒有建立縮圖。NPR-27122：CQ-4255080 的 Hotfix

#### 網站 {#sites-6}

* 暫停上層頁面會從遺失的頁面中移除cq：LiveRelationship mixin型別。 NPR-26996：CQ-4254113 的 Hotfix
* (外部連結檢查程式) 內部連結在個別頁面上顯示為已損毀，但對外部連結沒有相同作用。NPR-27481：CQ-4257780 的 Hotfix
* 編輯其他頁面屬性時，雲端服務設定繼承關係中斷。NPR-27311：CQ-4256785 的 Hotfix
* 搭配Foundation表單使用核心元件時出現Null指標例外狀況。 NPR-27333：CQ-4249176 的 Hotfix
* RTF 編輯器移除了空白的 alt 標記。NPR-26938：CQ-4253267 的 Hotfix
* (傳統 UI) 選取的效能問題，如果有多個下拉式清單已變更接聽程式。NPR-27115：CQ-4237215 的 Hotfix
* RTF編輯器與多個欄位結合時，出現「Uncaught TypeError： fieldAPI.getName不是foundation.js的函式」錯誤。 NPR-27146：CQ-4253155、CQ-4259967 的 Hotfix
* 即使按一下Safari瀏覽器中的選項按鈕，焦點/游標仍停留在RTF編輯器上。 NPR-27144：CQ-4249635 的 Hotfix
* 使用者嘗試編輯內容片段時頁面顯示為空白。 NPR-27669
...
* 在Dynamic Media元件中指定中斷點後，影像沒有顯示。 CQ-4256168 的 Hotfix

#### DAM - DMServices {#dam-dmservices}

* MixedMediaSet 與相關視訊沒有正常同步。CQ-4251650 的 Hotfix
* 視訊不會在「混合媒體集」的檢視器預設集編輯器中播放。 CQ-4251442 的 Hotfix

#### DAM - 一般 {#dam-general-1}

* 套用SP3修補程式後遺失內容片段模型的連結。 CQ-4259029 的 Hotfix

#### DAM - UI {#dam-ui}

* 安裝SP3後資料夾中繼資料結構UI毀損。 CQ-4257737 的 Hotfix

#### 社群 {#communities-5}

* 新增分頁支援以分組發佈時列出的頁面。NPR-26953：CQ-4246525 的 Hotfix
* 未讀通知數不能設為超過 21。NPR-27496：CQ-4252829 的 Hotfix
* 使用者訂閱限制上限為1000。 NPR-27615：CQ-4254689 的 Hotfix
* 在論壇中上傳影像以外的附件 (例如 .pdf) 時發現錯誤。NPR-27375：CQ-4257753 的 Hotfix
* 用於回覆的連結對列點選無效。 NPR-24556：CQ-4256138 的 Hotfix
...
* 啟用Scorm資源完成狀態在UI中顯示為空白。 NPR-27152：CQ-4249994 的 Hotfix
* 啟用&#x200B;**允許有特殊許可權者**&#x200B;時，有特殊許可權的成員應該只能為具有社群成員身分的使用者撰寫內容。 NPR-27901：CQ-4251235 的 Hotfix
...
* 在使用Tomcat的AEM WAR中，「儲存並關閉」頁面屬性沒有傳回正確的頁面。 NPR-27567：Granite-23671 的 Hotfix

* 儲存後，透過sourceEdit功能輸入的文字會遺失。 CQ-4259273 的 Hotfix
...
* 在AEM Forms頁面中設定了錯誤的表單時，AEM Sites容器元件會隱藏，且會選取「表單涵蓋整個頁面寬度」核取方塊。 NPR-25972：CQ-4239287、CQ-4249133 的 Hotfix
...
* 啟用IDS分離後，DAM更新資產工作流程不再連結參考。 NPR-26135：CQ-4250933 的 Hotfix
* 無法開啟解封存器步驟所建立的封存檔中名為%的資料夾的解壓縮。 NPR-26275：CQ-4251482 的 Hotfix
...
* 資產連結共用不允許新增超過一名使用數值使用者ID的內部使用者。 NPR-26206：CQ-4251466 的 Hotfix
* 以 deflate64 演算法壓縮的 zip 檔案損毀。NPR-26793：CQ-4253995 的 Hotfix
* 複雜PDF檔案的縮圖產生程式無法正常運作，導致部分影像遺失的縮圖。 NPR-26057：CQ-4250944 的 Hotfix
* 產生縮圖時出現堆積記憶體利用問題。NPR-25545：CQ-4246960 的 Hotfix
* 在資產上建立許多關係會導致錯誤。NPR-26309：CQ-4250708 的 Hotfix
* **刪除轉譯**&#x200B;選項無法運作，且擲回「無專案可刪除」錯誤。 NPR-26007：CQ-4213414 的 Hotfix
...
* 原則不會控制「編輯」對話方塊中的RTF編輯器功能。 NPR-27059、NPR-26750：CQ-4241130 的 Hotfix
...
* 新增「特殊字元」會將RTF編輯器對話方塊捲動至頂端。 NPR-26435：CQ-4249869 的 Hotfix
...
* 結構化內容片段模型標籤欄位不反映標籤移動操作。 NPR-26801：CQ-4251805 的 Hotfix
...
* 將變數建立為Live Copy需要使用者重新整理頁面才能反映。 NPR-26663：CQ-4254328 的 Hotfix
* （傳統UI）返回頁面屬性，縮圖影像不再使用繼承專案，並從網站管理員和Sidekick中消失，而影像顯示為空白。 NPR-26562：CQ-4252346 的 Hotfix
...
* 復寫使用過期的OAuth代號，造成多個錯誤。 NPR-25894：GRANITE-22388 的 Hotfix
...
* 存取存取權杖節點時，多個復寫代理程式同時存取會導致記錄中出現例外狀況。 NPR-26964：GRANITE-23276、Granite-23155 的 Hotfix

#### ContextHub {#contexthub}

* 檔案`coral.js`包含易受攻擊的程式庫`handlebars.js`版本。 CQ-4255377 的 Hotfix
...
* 建立論壇貼文時，附件影像未顯示縮圖。 NPR-26380：CQ-4253105 的 Hotfix
* 從Word外掛程式貼上的操作無法載入一個以上的影像。 NPR-26728：CQ-4253638 的 Hotfix
* 無法篩選審核頁面中的內容。NPR-26697：CQ-4213766 的 Hotfix
* (安全漏洞) 由於 JWT 錯誤設定，帳戶遭接管。NPR-26440：CQ-4253314 的 Hotfix
* 從Adobe儲存資源提供者擷取資料的速度緩慢。 NPR-26237：NPR-24152 的 Hotfix
...
* 在論壇元件上搜尋時發生問題。 NPR-26577：CQ-4251303 的 Hotfix
* 在社群訊息中同時搜尋名字和姓氏時沒有傳回預期的結果。NPR-26377：CQ-4253150 的 Hotfix
* 移除回覆時分頁沒有更新。NPR-26327
* 刪除貼文會生效，但控制檯會出現錯誤，且貼文仍可在UI上看到。 NPR-26292：CQ-4252803 的 Hotfix
...
* 將群組和成員新增至「角色」設定，以及允許有特殊許可權的成員使用部落格功能時，出現Null指標例外狀況。 NPR-21732、NPR-27176：CQ-4255909 的 Hotfix
* 編輯網站並在角色設定中新增成員時，系統擲回Null指標例外狀況。 NPR-27132：CQ-4255909 的 Hotfix
...
* 無法建立或編輯Document Security的水印設定。 NPR-26267、NPR-26129：CQ-4250234 的 Hotfix
...
* 編輯並儲存可編輯的模組，然後開啟或關閉「PDF預覽」，可保持可見的PDF和信件「HTML預覽」。 兩個預覽在相同視窗中仍可正常運作。 NPR-26770：CQ-4253217 的 Hotfix

#### Forms - 工作流程 {#forms-workflow-2}

* 工作區會斷斷續續地中止並重複顯示起始點。 NPR-26461：CQ-4253439 的 Hotfix
* 任務名稱中使用大括號時，系統在記錄中擲回 ESAPI 例外狀況。CQ-4256627 的 Hotfix
* 開啟與起點相關聯的非預填適用性表單或表單集時，會擲回Null指標例外狀況。 CQ-4256124 的 Hotfix
* 無法使用全域設定傳送電子郵件。NPR-26163：CQ-111110 的 Hotfix
* 套用axis.jar檔案後無法開啟工作區。 NPR-26131：CQ-4251126 的 Hotfix
* 重新整理按鈕無法將最新資料從伺服器同步至適用性表單。CQ-4255670 的 Hotfix
* 從管理員UI設定搜尋範本，然後在AEM Forms工作區中使用相同的範本進行搜尋時，系統擲回例外狀況。 CQ-4255649 的 Hotfix
* 應用程式名稱中若包含括弧，其底下的程式追蹤詳細資訊不會顯示在HTMLWorkspace中。 CQ-4242101 的 Hotfix
* 在HTMLWorkspace的偏好設定畫面中儲存變更時，系統擲回例外狀況。 CQ-4241485 的 Hotfix
* 最新 JEE 設定的大量核准功能失效。CQ-4241486 的 Hotfix
* 最新 6.4 JEE 伺服器上的任務/起始點草稿失敗。CQ-4241484 的 Hotfix
* 設定Date()。getTime()。toString引數以初始化工作階段而非Date.toString()。 CQ-4241483 的 Hotfix
* 開啟/lc/ws時，在HTML引數中發現有漏洞的字元例外狀況。 CQ-4241481 的 Hotfix
...
* 無法將esapi.properties載入為WAS9中的類別載入器資源。 CQ-4255631 的 Hotfix
...
* 修正PDFG靜態程式碼分析報表中報告的問題。 NPR-23251：CQ-4213930 的 Hotfix
...
* 將com.adobe.granite.ui.coralui3-rte更新至已發行的最新版本。
* 已確認如果expressionResolver為null，則轉譯條件會傳回合理的結果。
...
* 與顯示的資訊相比，下載大小不相符。 NPR- 23418：CQ-4242774 的 Hotfix
...
* 預覽後搜尋面板中的資產管理員搜尋Facet消失。 NPR-23964：CQ-4244410 的 Hotfix
...
* Publish/取消發佈至Brand Portal按鈕不應適用於內容片段。 NPR-23318：CQ-4245086 的 Hotfix
* (Brand Portal) 啟用發佈資產時建立子資產的功能。NPR-23331：CQ-4242018 的 Hotfix
* Dynamic Media請求不使用Proxy/HTTP常見使用者端。 NPR-10727：CQ-45695、CQ-88800 的 Hotfix
* 無法在Dynamic Media S7 (DMS7)中為單一轉譯MP4視訊資產加上註釋。 NPR-22046：CQ-4215912 的 Hotfix
* EmbedXMP 資料一律對 TIFF、pyramid 產生程序設為「作用中」。NPR-22903：CQ-4234498 的 Hotfix
* 大量影像預設集出現動態轉譯顯示/選取問題。 NPR-23151：CQ-4217511 的 Hotfix
* Dynamic Media編碼視訊 — 修改/重新上傳啟動器出現問題。 NPR-23237：CQ-4240260 的 Hotfix
* Dynamic Media S7 中 HTTP 轉寄站的 Proxy 處理修正。NPR-24001：CQ-244140 的 Hotfix

### 網站 {#sites-8}

* 6.2和6.3之間因不同xPath轉譯而產生的查詢產生器差異。NPR-23245：CQ-4240396的Hotfix
* 在延伸對話方塊時，頁面屬性中的縮圖索引標籤無法運作。 NPR-22844：CQ-4241474 的 Hotfix
* Parsys 中斷了模擬器裝置框架寬度，並裁切掉其中新增的任何元件。NPR-22926：CQ-4238224 的 Hotfix
* 執行多次啟動時，會在Author中提升該啟動，但由於缺少復寫許可權，因此不會在Publishing伺服器上復寫變更。 NPR-22934：CQ-4234746 的 Hotfix
* 不同工作階段中的另一個使用者可以修改使用者在第一個工作階段中鎖定的頁面。 NPR-23057；CQ-4199017 的 Hotfix
* 修正清單檢視中的可重新排序選項。 NPR-23065：CQ-4239321 的 Hotfix
* （頁面編輯器）再次開啟對話方塊時，影像元件上的影像會消失。 NPR-23156：CQ-4239978 的 Hotfix
...
* 「XF變化」的「Blueprint」索引標籤中缺少轉出按鈕。 NPR-23320：CQ-4240404 的 Hotfix
* 由於功能遭淘汰，傳統 UI 無法用於編輯 CUG。NPR-24122：4241823 的 Hotfix
* 用於防止提升不需要的內容之主動式修正。NPR-24387：4244993 的 Hotfix
* 一旦將大約 80 個在Assets的資料夾中新增80個片段，從時間軸主控台觸發工作流程時會遇到錯誤。 NPR-23393；CQ-4211216 的 Hotfix
* 無法從內容尋找器將影像拖放至RTF編輯器對話方塊中。 NPR-23403：CQ-4242094 的 Hotfix
* 將元件從 AEM 6.0 移轉到 AEM 6.2 時，出現「無效的遞迴選擇器值」錯誤。NPR-23532：CQ-4241258 的 Hotfix
* （RTF編輯器）工具提示顯示外掛程式的變數名稱，而非可讀的外掛程式名稱。 NPR-23550：CQ-4243269 的 Hotfix
...
* 如果在搜尋Facet中使用標籤述詞，儲存後智慧型集合不會顯示結果。 NPR-23401：Granite-21278 的 Hotfix
* 修補 clientlib 中 jQuery 1.12.4 以包含安全性修正。NPR-24128：Granite-20058 的 Hotfix
* 現在當套件組合重新啟動時，國際化翻譯會進行更新。 NPR-23193：Sling-7190 的 Hotfix
...
* (Target)使用XML等API型別時，不會啟用行銷活動。 NPR-23199：CQ-4240936 的 Hotfix****
* 中繼資料夾結構導致設定參考復寫失敗。 NPR-23485：CQ-4242751 的 Hotfix
...
* 建立語言副本時不會刪除Live Copy屬性。 NPR-23123：CQ-4230641 的 Hotfix

### 使用者介面 {#user-interface-1}

* DatePicker不支援手動設定隱藏欄位所設定的外部型別提示。 變更類型提示會出現對話錯誤。NPR-23371：Granite-21194 的 Hotfix
* Coral.ColumnView：新增對 Shift + 按一下操作的支援。NPR-23404：Granite-13338 的 Hotfix
* 選取了具有空白值的專案時，選取RT不會進行驗證。 NPR-23405：Granite-21283 的 Hotfix
...
* 在社群中搜尋因幾個字而失敗。 NPR-23256：CQ-4240717 的 Hotfix
* 無法為群組管理員角色指派群組的問題。NPR-23317：CQ-4241233、CQ-4221399 的 Hotfix
* 以類似的資產名稱建立資源時出現問題。NPR-23319：CQ-4240700 的 Hotfix
* (ContextPath)中斷訊息功能因Jetty設定而中斷，且在社群群組成員清單中搜尋群組成員時發生錯誤。 NPR-23336：CQ-4241519、CQ-4242080 的 Hotfix
...
* 已取消標幟的使用者之標幟理由仍會顯示。 NPR-23974：CQ-4243822 的 Hotfix
...
* 請求機制在處理後或定期從背景工作執行個體中清除資產。 NPR-23638：Granite-21337 的 Hotfix
...
* 數值欄位不遵循最小值。 NPR-23967：CQ-4244830 的 Hotfix
...
* 套用新axis.jar檔案後無法開啟工作區。 NPR-23316
...
* 變更資產資料夾設定中的標題或縮圖圖片時，原始群組和資料夾的許可權會被覆寫。 NPR-22171：CQ-4216080 的 Hotfix
* UI擲回假錯誤「Publish到Brand Portal失敗」，其實工作已新增到復寫佇列且資產已發佈到Brand Portal。 NPR-22179：CQ-4205273 的 Hotfix
* (觸控式 UI) 欄檢視中資產的預設上傳位置。NPR-22465：CQ-4237057 的 Hotfix
* 嘗試將資產結構描述從`/conf/global`複製到`/conf/mytenant`時，AEM擲回StackOverflow錯誤。 NPR-22489：CQ-4235875 的 Hotfix
...
* 當一個 PDF 資產有多個相關的關鍵字時，系統無法妥善擷取和正確修改 PDF 關鍵字中繼資料。為了解決問題，已移除 PDF 資產的「主旨」欄位中繼資料屬性。不過，您可以編輯中繼資料結構來為「主旨」欄位新增多值文字欄位。 NPR-21972：4215741 的 Hotfix****
...
* 透過 Safari 在 Assets 中使用 Digital Rights Management 時，資產下載失敗。使用者無法下載含有免責宣告和檔名很長的資產。 NPR-22747：CQ-4236460 和 CQ-4235274 的 Hotfix
* 將公開共用設為將資產發佈至Brand Portal時的預設選項。 NPR-21931：CQ-4218816 的 Hotfix

### 網站 {#sites-9}

* 工作流程收件匣中的新專案顯示頁面路徑而非頁面標題。 NPR-21634：CQ-4230672 的 Hotfix
* 可編輯的結構元件遺失編輯時回應式格線所需的CSS類別名稱。 NPR-21741：CQ-4232374 的 Hotfix
* （觸控式UI） HTL元件有多個跨網站指令碼(XSS)漏洞。 NPR-21899：CQ-4232511 的 Hotfix
* 無法變更內容片段混合媒體影像資源型別。 NPR-21907：CQ-4233401 的 Hotfix
* 觸控式UI對話方塊的RTE全熒幕隱藏了RTE外掛程式。 NPR-22034：CQ-4235457 的 Hotfix
...
* （傳統UI） RTF編輯器輸出html將「list-position-style： inside；」作為內嵌樣式新增到&lt;ul>標籤中。 NPR-22145：CRTE-114 的 Hotfix
* 當文字空白時，讓TreeNode回復到name屬性。 NPR-22146：CQ-4234724/CQ-4236300 的 Hotfix
...
* 輸入文字時，標籤欄位的篩選功能未如預期運作。 NPR-22236：CQ-4236655 的 Hotfix
...
* 初始化期間ContextHub儲存覆寫預設持續層時，出現持續性問題。 NPR-22479：CQ-4218399 的 Hotfix
* 如果沒有在第一個內容根勾選「包含子頁面」，含有多個頁面的啟動就不會將子頁面發佈至發佈伺服器。 NPR-22482：CQ-4237818 的 Hotfix
* （觸控式UI）透過傳統UI控制檯刪除啟動會導致所有頁面都無法編輯。 NPR-22491：CQ-4225074 的 Hotfix
...
* 將新增至頁面上影像元件中的影像翻轉後，變更不會儲存且原始影像會顯示在頁面上。 透過 [https://github.com/adobe/aem-core-wcm-components/pull/141](https://github.com/adobe/aem-core-wcm-components/pull/141) 為影像核心元件新增轉譯支援。NPR-22801：CQ-4221539 的 Hotfix
* 當使用者嘗試從錨點選單中刪除現有錨點時，RTF編輯器元件視窗會關閉，且變更保持未儲存狀態。 NPR-22802：CQ-4238167 的 Hotfix
* Omnisearch篩選器沒有顯示Sites主控台中的所有動作。 NPR-22804：CQ-4239007 的 Hotfix
* 觸控式 UI 中 OS 剪貼簿和內部 AEM 剪貼簿出現複製/貼上問題。NPR-22807：CQ-4220383 的 Hotfix
* Lucene 搜尋傳回的摘要醒目提示不一致。NPR-22879：CQ-4238513 的 Hotfix
* 使用發佈例項啟動頁面會導致綠色狀態而非變為琥珀色。 NPR-22927：CQ-4236310 的 Hotfix
* (StyleSystem) 從快顯視窗選擇樣式時螢幕位置跳躍。NPR-23183：CQ-4238867 的 Hotfix
* （管理發布）需按幾下滑鼠，才能移至日曆的下一個月。 NPR-23508：CQ-4242732 的 Hotfix

### 平台 {#platform-2}

* Sling Exporter servlet無法正確匯出日文字元。 NPR-22153：CQ-4228920 的 Hotfix
...
* 目標引擎(mbox.js、at.js)不使用損毀URL。 而且會使用包含冒號的URL，某些URL部署可能會失敗。 NPR-22366：CQ-4237854 的 Hotfix
* 提供自訂at.js或mbox.js時，系統會將包含的指令碼以文字形式寫入頁面，而非以HTML標籤寫入。 NPR-22441：CQ-4203691 的 Hotfix
* 在「目標」模式中，作者不需要取消繼承即可修改從 Blueprint 繼承的元件。NPR-22751：CQ-4237907 的 Hotfix
* 由於遺失`jcr:content`節點，PersonalizationDataSource擲回Null指標例外狀況。 NPR-22850：CQ-4222122 的 Hotfix
* 使用非英語時，AEM鎖定目標會失敗。 NPR-22917：CQ-4218213 的 Hotfix
* 含有目標內容的發佈頁面遺失相關資源。NPR-23064：CQ-4227119 的 Hotfix
* 使用者無法檢視mbox呼叫中的測試「靜態引數」值，而透過雲端設定中作為使用者端程式庫的AT.js進行測試時可看到這些值。 NPR-21930：CQ-4234520 的 Hotfix

### WCM-Foundation 元件 {#wcm-foundation-components-1}

* 取消勾選「取消/停用繼承」核取方塊後，iParsys會建立位置移位。 NPR-21905：CQ-4230951 的 Hotfix
* 表單下拉式元件的顯示/隱藏功能未如預期運作。NPR-22327：CQ-4222853 的 Hotfix
* 為提高安全性，已棄用CAPTCHA元件。 若您使用CAPTCHA元件，安裝6.3.2.1或更新版本後，訊息會顯示「已棄用Captcha元件且不應再使用」。 為提高安全性，AEM 元件可供自訂以包含 reCAPTCHA NPR-22151：CQ-4220052 的 Hotfix

### WCM - 頁面編輯器 {#wcm-page-editor}

* 使用無效選擇器時出現反射式跨網站指令碼(XSS)。 CQ-4270397 的 Hotfix
...
* （傳統UI）即使相關的表單資料模型服務設定為空白欄位，元件仍會顯示預設值。 NPR-22272：GRANITE-19744 的 Hotfix
...
* 啟動時LogbackManager可能會遺漏某些OSGi設定。 NPR-23060：Granite-19791 的 Hotfix

### 商務 {#commerce-3}

* 啟用在體驗片段功能表中建立工作流程。 NPR-22347：CQ-4221661 的 Hotfix
...
* 新增產生PDF/A-2a或PDF/A-3a相容PDF檔案的支援。

### Forms 附加元件套件 {#forms-add-on-package-9}

#### Forms - 管理員 UI {#forms-admin-ui}

* 當您在「ECM聯結器設定」頁面上檢查「密碼」欄位時，密碼會以純文字顯示。 NPR-22508：CQ-4236065 的 Hotfix
...
* 規則編輯器的自訂函式在函式呼叫後新增額外的；。 因此，即使自訂函式傳回true，它們仍會失敗驗證。 NPR-22481：CQ-4235499 的 Hotfix
* 顯示驗證訊息下限和上限時，無論選取的日期模式為何，日期選擇器元件都不會依循該模式。 NPR-22444：CQ-4236269 的 Hotfix
...
* (Microsoft® Edge) (IE11)具有多行欄位的最適化表單文字欄位元件會將`Null`顯示為預設值而非空白。 NPR-22284：CQ-69107 的 Hotfix
* 適用性表單中的 SOAP UTF-8 輸入編碼傳回錯誤並出現亂碼頁面。NPR-20105：CQ-4222669 的 Hotfix
* 一旦在AEM Forms頁面中設定了錯誤的表單，AEM Sites容器元件就無法用於編輯。 CQ-4237456 的 Hotfix
* 在JEE伺服器上執行開發測試時失敗。 CQ-4222082 的 Hotfix
* 由於Calvin框架中的縮制問題，JEE伺服器上的AF測試失敗。 CQ-4217220 的 Hotfix

#### Forms Manager {#forms-manager}

* (Firefox) 因為文件記錄 (DOR) 中的選項不是屬性頁面中的預先選取項目，因此無法更新適用性表單的 XML 結構屬性。NPR-22298：CQ-4237402 的 Hotfix
* 發佈頁面後修改過的Forms不會再發佈。 NPR-23013：CQ-4236566 的 Hotfix
...
* (HTMLWorkspace)在工作區中關閉表單時會觸發準備資料。 NPR-21127：CQ-4224574 的 Hotfix
* (HTML 工作區) 叫用需要含有長說明的附註之程序時，附註展開按鈕無法運作。CQ-4241488 的 Hotfix

#### 核心 {#core-1}

* 啟動排程器期間在啟始時發生錯誤。NPR-22990
* 瀏覽器無法儲存輸入至HTML表單的認證。 NPR-21762：CQ-4206543 的 Hotfix
...
* 新增產生PDF/A-2a或PDF/A-3a相容PDF檔案的支援。 NPR-23021：CQ-4214172 的 Hotfix
...
* 從Designer另存為PDF時，Designer中定義的內嵌超連結沒有受到標籤，或具有替代文字。 NPR-23133
...
* 透過HSM以數位方式簽署PDF檔案時發生例外狀況。 NPR-21154：CQ-4226978 的 Hotfix
...
* 使用者無法下載檔案名稱長和含有已定義免責宣告的資產。 NPR-20254
* ProductAssetsUploader會將Java™快取的暫存檔放在Java™ TEMP資料夾中。 NPR-20256：CQ-4221801 的 Hotfix
...
* OmniSearch中有過多字元可能會導致AEM伺服器當機。 NPR-20434：CQ-4223602 的 Hotfix
* DAM資產API中繼資料缺陷破壞了xmp-write-back API。 NPR-20607：CQ-4220455 的 Hotfix
...
* AEM Dynamic Media Classic (前身為Scene7)無法從AEM伺服器同步資產。 當Cloud Service設定中的目的地路徑指向根路徑子資料夾時，就會發生此問題。 CQ-4228265
* 新增 Apache Commons `{org.apache.commons/commons-email/1.5}` 的電子郵件套件組合取代了 `{com.day.commons.osgi.wrapper/com.day.commons.osgi.wrapper.commons-email/1.2.0-0002}`。

### 網站 {#sites-10}

* 管理員權限問題：無法從頁面屬性中移除「已關閉的使用者群組」成員。NPR-20631
* 透過「管理發布」發佈頁面時，輸入的工作流程名稱應該可在「通知」方塊中使用。 NPR-20046：CQ-4221586 的 Hotfix
* 在 RTF 編輯器中對兩列套用已設定樣式的文字，然後將其合併到一個段落中，會移除已設定樣式的範圍。NPR-20110：CQ-4223051 的 Hotfix
* 在「屬性」編輯對話框中，對多個索引標籤中的欄位屬性所做的變更沒有儲存。NPR-20286
* 內容片段中貼上的內容結尾出現未預期的標籤。 NPR-20413：CQ-4224014 的 Hotfix
* 在 Adobe Campaign 中實作一套機制，用於從外部目標定位資源擷取內容資源的原則。NPR-20667
* RTF外掛程式無法用於多欄位觸控式UI中的內嵌和全熒幕列。 NPR-20973
...
* 搜尋結果元件的搜尋結果不一致。NPR-20070：CQ-4220913 的 Hotfix
...
* 建立Communities網站後，使用者無法從網站地圖開啟該網站，因為內容路徑未加到URL前面。 NPR-20730
...
* Search&amp;Promote雲端服務設定會忽略表單擷取方法，導致其無法用於製作執行個體。 NPR-20447：CQ-4206076 的 Hotfix
* 安裝Search&amp;Promote功能時，內容套件中模稜兩可的篩選器定義會導致路徑遭到覆寫。 NPR-20808

### Mobile On-Demand {#mobile-on-demand}

* TXT型別資產的中繼資料屬性每次顯示時，都會顯示在不同的中繼資料編輯器中。 NPR-20239

### 平台 {#platform-3}

* 解決無結尾標記的資源解析器例外狀況。NPR-19749：Granite - 19143 的 Hotfix
* 請求在操作控制面板中隨著預設健康狀態檢查顯示自訂卡片。NPR-20145
* 頁面編輯器的附註層在Internet Explorer 11中無法正常運作。 NPR-20222：CQ-4222818 的 Hotfix
* 在複寫代理中將使用者代理設為管理員時發生工作階段洩漏。NPR-20578
* 現在已為其他data-sly-resource陳述式取消設定requestAttributes。 NPR-20639：4226206 的 Hotfix
* 修正 AEM 中針對 OOTB 資產的 curl Head 請求問題。NPR-20781：CQ-4221520 的 Hotfix

### 專案 {#projects-1}

* 從「專案」主控台存取不同專案時需花較長時間載入。 NPR-20314
* 安裝 AEM 6.3.0.1 會刪除 `dam-update-service` 使用者的金鑰存放區。NPR-20018
* 在某些自訂部署中，使用者若嘗試在addTask模組中選取受託人，會花較長的時間填入使用者選取器中的清單。 NPR-20283：CQ-4224193 的 Hotfix

### 使用者介面 {#user-interface-3}

* 無論對話方塊中的屬性為何，「顏色」欄位皆設為「永遠必要」。 NPR-19702
* Internet Explorer 11 中沒有以全螢幕為多欄位元件顯示捲動軸。NPR-20261：CQ-4219782 的 Hotfix
...
* 無法將Switch元件的值設定為true或false。 NPR-21111
...
* 如果沒有在協助工具設定中定義名稱，XDP表單物件（例如子表單）的名稱會顯示為其工具提示。 NPR-20523

### Forms - JEE 安裝程式 {#forms-jee-installer-9}

#### 處理程序管理 {#process-management-2}

* FormSetPrefillApp起點未預先填入AEM Forms應用程式中的表單集欄位。 NPR-20950
...
* 針對預設中繼資料結構表單啟用發佈至Brand Portal動作。
* 針對dc：title屬性設為`String [] (multifield)`的影像，在影像卡片上顯示標題的行為變更。
* 以基礎時間元件取代伺服器端的時間轉譯。
* 啟用透過標籤管理員/標籤控制檯將標籤從AEM發佈至Brand Portal的功能。
* 發佈JSON API以使用內容片段。
* 內建存放庫 (Apache Jackrabbit Oak) 更新至 1.6.5 版。

### 資產 {#assets-11}

* 將具有相同屬性的兩個欄位對應到不同屬性欄位型別時，系統擲回內部錯誤。 NPR-19462：CQ-4216828 的 HF
* dc:title 和 dc:description 不會變更為 CRXDE Lite 中的多欄位值。NPR-19570：CQ-4209086 的 HF
* Dynamic Viewer 載入畫質最低的視訊轉譯，用於以製作模式測試視訊播放體驗。NPR-19004
* 無法為名稱中包含空格的資產下載動態轉譯。NPR-19433：CQ-4211738 的 Hotfix
* 無法使用Chrome在欄檢視中載入頁面/資產的完整清單。 NPR-19566：CQ-4214248 的 Hotfix
...
* 設定精靈中的「鎖定目標引擎」欄位設為ContextHub (AEM)，而非Adobe Target。 NPR-19320：CQ-4218465 的 HF
* 建立體驗時，對象區段損毀。 NPR-19110
...
* 在製作對話方塊中，粗體、斜體、底線功能無法用於RTF編輯器欄位。 NPR-19670：NPR-19718：CQ-4219088 的 Hotfix

### Mobile On-Demand {#mobile-on-demand-1}

* AEM文章控制檯出現轉譯問題，使用完整大小的影像導致其速度變得緩慢。 NPR-19088
...
* 升級至AEM 6.3後，找不到參考標籤實作為指令碼的自訂標籤程式庫。NPR-19087
...
* 選取XF變化編輯器後，「開始工作流程」按鈕沒有作用。 NPR-19642：CQ-4207796 的 Hotfix

### 專案 {#projects-2}

* 專案編輯器無法將資產複製並貼到專案資產資料夾中。 NPR-19619：CQ-4215321 的 Hotfix

### Web 內容管理 {#web-content-management}

* 在「轉出」畫面中，無法勾選或取消勾選與「即時副本」頁面對應的核取方塊。 NPR-19518
* 因為目前所有索引標籤和欄位都可進行大量編輯，因此無法正常使用頁面屬性大量編輯功能。NPR-19451
* 在傳統UI中編輯影像時，OOTB屬性和影像對齊方式屬性沒有顯示。 NPR-19488：CQ-4217914 的 HF

### 使用者介面 {#user-interface-4}

* 使用Google Chrome瀏覽器時，使用者無法在觸控式UI中使用欄檢視來載入頁面/資產的完整清單。 NPR-19566：CQ-4214248 的 Hotfix

### Brand Portal {#brand-portal-1}

* 無法從 AEM 發佈包含評論和附註的資產。NPR-19590：CQ-4218386 的 Hotfix
* 啟用透過標籤管理員/標籤控制檯將標籤從AEM發佈至Brand Portal的功能。 NPR-20271：CQ-4223948 的 Hotfix
...
* 使用Cloud Service設定Google `reCaptcha`時，在需要org.apache.http.proxyconfigurator設定以進行外部連線的環境中，POST呼叫似乎不會通過PROXY。 NPR-20454
* 安裝時以XSD結構為基礎的調適型Forms會針對數值欄位提交有問題的XML值，且語言環境有小數分隔符號而非句點(.) 進而導致出現錯誤。NPR-20444
* 對第三方伺服器發出HTTP請求時，系統沒有遵循針對「Apache HTTP元件Proxy設定」所設定的Proxy設定。 使用 HTTP GET 或 POST 呼叫時出現連線逾時問題。NPR-20457、NPR-20456、NPR-20455、NPR-20451
...
* 當表單最大化/最小化並儲存為草稿或轉寄時，工作流程附件、備註和詳細資訊索引標籤在工作區中無法運作。 NPR-20243
* 在HTMLWorkspace中提交資料後，多行文字欄位(TextArea)沒有保留文字中的新行字元或分行符號。 NPR-20085

#### 程序報告 {#process-reporting}

* 由於Null指標例外狀況，程式報告功能無法正確擷取資料。 NPR-19759

>[!NOTE]
>
>安裝[AEM Forms發行版本](aem-forms-releases.md)文章中列出的LiveCycle內巢狀件以修正問題。
...
* 當 JBoss® 應用程式上以叢集設定執行的 AEM Forms Server 停止時，應用程式伺服器會中斷與資料庫的連線。 這可能導致資料損毀問題。NPR-19724

### 包含 Feature Pack {#feature-packs-included-1}

* 下拉式中繼資料結構欄位無法設為必要，因為資產缺少必要欄位驗證。 NPR-17882：CQ-4208373 的 FP
...
* 預設中繼資料結構導致內容片段驗證失敗。 NPR-17769：CQ-4211111 的 Hotfix
...
* 在混合裝置上使用Chrome編輯中繼資料結構表單時，在該表單中新增索引標籤會發生問題。 CQ-4201810
* 上傳重複資產時，如果沒有在偵測到重複專案的對話方塊中選取選項，則（刪除/保留）選項會套用至所有資產。 CQ-4201673
* 當資產資料夾中有超過 150 傳入的參考移動時，會出現 Null 指標例外狀況。CQ-4200981
* 針對已下載的資產資料夾，擷取 ZIP 檔案的內容時如果發生衝突，預設選項會顯示為「建立版本」而非「保留兩者」。CQ-97800
* 擁有應用程式唯讀許可權的使用者無法從AEM Mobile內容管理預覽內容。 NPR-17486：CQ-4209690 的 CFP
* 「目錄」主控台的欄檢視中，「建立目錄」按鈕無法運作。 CQ-4209952

### 網站 {#sites-12}

* 透過 data-sly-resource 屬性內嵌影像/視訊元件時發生問題。NPR-18182：CQ-4212100 的 CFP
* 在LiveCopy中重新套用繼承時，已修改的當地語系化元件不會還原為其原始表單。 NPR-18172：CQ-4211379 的 Hotfix
...
* 長頁面標題不會根據慣例產生頁面名稱。 NPR-17633：CQ-4209056 的 Hotfix
...
* 工作流程對話框欄位的值沒有保留在工作項目節點中。NPR-17734：CQ-4210369 的 Hotfix
...
* 通訊管理文字模組、信函預覽和建立通訊管理UI的程式化啟動中的修正。
* PDF 產生器中對 PDF/A-1b 驗證、大型影像檔案轉換為 PDF，以及日文 PDF 文件的修正。
* 通訊管理、檔案安全性和Forms Workflow的可用性修正。
* 新增支援以擷取手寫簽名欄位的稽核事件。
...
* 編輯通訊管理片段時，文字編輯器顯示內嵌條件以及已處理的文字。 CQ-4211930
...
* 驗證 PDF 文件的 PDF/A-1b 合規性時 AEM Forms 傳回驗證錯誤：表單欄位有多種外觀。PDF 文件符合 PDF/A-1b。NPR-18013
...
* PDF Generator服務無法將大於100 MB的影像轉換為PDF檔案。 CQ-4208628
* 搭配日文OCR使用PDF Generator服務時，會產生反轉PDF。 NPR-17602
...
AEM Cumulative Fix Pack 6.3.0.2 為重要更新，包含自 2017 年 4 月正式發行 AEM 6.3 以來累積的多項內部及客戶修正。AEM Cumulative Fix Pack 的關鍵重點為：
...
>[!NOTE]
>
>Adobe 提供的選用 Feature Pack 依存於發行版本和 Cumulative Fix Pack。若您已安裝任何 Feature Pack，聯絡 [AEM 客戶服務團隊](https://experienceleague.adobe.com/?support-solution=General#support)以驗證與這個適用 AEM 6.3 的 Cumulative Fix Pack 之間的相容性。

>[!NOTE]
>
>Adobe建議您在嘗試安裝之前，先對每個新的安裝套件執行驗證。 安裝之前預先驗證程序會分析並報告所有發現的錯誤，並主動警告使用者出現這些錯誤。
>
>您可以在[https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)存取「驗證」選項的檔案

* AEM 6.3.3.0 是 CFP 的必備條件。請參閱[升級檔案](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)，取得有關將AEM安裝升級為AEM 6.3的詳細指示。
* 對於使用 RDBMK 或 MongoDB 的叢集部署，CFP 套件可安裝在使用套件管理器的任何製作執行個體上。
...
activity.log會建立在crx-quickstart /logs資料夾中。
...
>[!NOTE]
>
>如果您要安裝6.3.3.0或更新版本，請執行以下程式來配置JBoss®應用程式伺服器的設定。 若您是在 Oracle WebLogic 或 IBM® WebSpehere 應用程式伺服器上所執行的 AEM Forms Server 上安裝 6.3.3.0，則無需進行額外設定。如需更多詳細資訊，請參閱 [AEM 6.3.3.0 發行說明](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)。
...
* 安裝 AEM CFP 6.3.3.x 期間可能會出現下列錯誤和警告，且您可安全忽略這些訊息並繼續：

   * &#42;警告&#42; [OsgiInstallerImpl] org.apache.jackrabbit.vault.packaging.impl.InstallHookProcessorImpl Hook /META-INF/vault/hooks/cloudservices-wfchangeinstallhook-0.0.2-jar-with-dependencies.jar 擲回執行時間例外狀況。
   * &#42;錯誤&#42; [OsgiInstallerImpl] com.adobe.cq.social.cq-social-jcr-provider [com.adobe.cq.social.provider.jcr.impl.SpiSocialJcrResourceProviderImpl(2174)] 等待登錄變更以完成解除登錄逾時。CQ-4209974。
   * org.apache.sling.engine.impl.SlingRequestProcessorImpl ServletResolver服務遺失，無法服務要求，傳送狀態503
   ...
   * &#42;錯誤&#42; [FelixDispatchQueue] com.day.cq.dam.cq-dam-core FrameworkEvent ERROR (org.osgi.framework.ServiceException：服務工廠傳回 null。(元件：com.day.cq.dam.handler.standard.ps.PostScriptHandler))

**Brand Portal**

* 自6.3.1.2開始，移除了智慧型集合的「Publish至Brand Portal」按鈕。

**使用者介面**

>[!NOTE]
>
>若您受到這兩個問題其中任何一個的影響，請聯絡 [AEM 客戶服務](https://experienceleague.adobe.com/?support-solution=General#support)。

* 由於管理搜尋功能中有許多請求，出現高CPU使用率問題。 NPR-24229
...
若要手動更新 adobe- LiveCycle® cq -author.ear/ cq .war 中的變更，請依照下列步驟操作：
...
* 請依照 [https://helpx.adobe.com/tw/pdf/aem-forms/6-3/install-single-server-websphere.pdf](https://helpx.adobe.com/tw/pdf/aem-forms/6-3/install-single-server-websphere.pdf) 中 10.2 節提供的步驟操作，從 WebSphere® 應用程式伺服器解除安裝先前的 Enterprise Archive (EAR) 檔案，並安裝更新版 EAR 檔案。
* 儲存檔案並重新啟動伺服器。[https://helpx.adobe.com/tw/pdf/aem-forms/6-3/install-single-server-websphere.pdf](https://helpx.adobe.com/tw/pdf/aem-forms/6-3/install-single-server-websphere.pdf)

## NPR-23208 所需的配置設定 {#configuration-settings-required-for-npr-1}

>[!NOTE]
>
>此配置設定適用於 6.3.2.2 和更新版本。它會指引您透過CRX-DE手動更新存取控制清單(ACL)原則，因為由於「merge_preserve」acHandling，ACL不會透過CFP安裝更新。
...
型別：允許
許可權：jcr：read、jcr：modifyProperties
...
`2)` 在 &quot;/content/user generated&quot; 路徑
`a)` 主體：reference- adjustment-service
類型：允許
權限： `jcr:write`
...
預設值為 1。覆蓋檔案constants.js (`/libs/cq/ui/widgets/source/constants.js`)，取消註解`CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL`屬性以覆寫值。 然後，為其指派所需的值（儲存頁面屬性資料的頁面`/jcr:content`底下的深度上限）。

**如果使用者必須建立許多頁面變體，以至於頁面 `/jcr:content` 節點之下的節點數超過 1000，請使用下列步驟來進行設定變更：**

* 設定Apache Sling的JSON Max結果屬性。
* 使用`/system/console/ configMgr`取得Servlet。
* 將其值設為 >1000 (目前的預設值)，讓此數大於 `/jcr:content` 子樹狀結構中所設定深度之上的總結點數。

此程式可讓SlingGETServlet能夠傳回所有必要的節點。

## Uber Jar {#uber-jar}

適用於6.3.3.8的Uber Jar可於Adobe公共Maven存放庫取得

若要在Maven專案中使用Uber Jar，請參閱[如何使用Uber Jar](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)，並在您的專案POM中加入以下相依性：
...
| 區域 | 功能 | 替代方案 | 版本 |
|----|-----|-----|-----|
| Assets 與 Adobe Creative Cloud 整合 | AEM 6.2 引入了 [AEM 對 Creative Cloud 資料夾共用](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)功能，作為讓 Creative 使用者存取 AEM 資產的方式。Creative Cloud應用程式推出的新功能Adobe Asset Link提供更佳的使用者體驗。 它也能讓您直接從Photoshop、InDesign和Illustrator記憶體取更強大的AEM資產。</br></br> Adobe 不會對資料夾共用功能提供更多增強項目。由於此功能包含在 AEM 中，建議客戶使用替代功能。 | Adobe Asset Link 或桌面應用程式。如需詳細資訊，請參閱[AEMCreative Cloud整合](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)文章。 | AEM 6.3.3.x |
...
>[!MORELIKETHIS]
>
>* [AEM 發行版本和更新](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)
>* [AEM 6.3 Hotfix 頁面](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)
>* [AEM 6.3 發行說明](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [AEM 產品頁面](https://business.adobe.com/products/experience-manager/adobe-experience-manager.html)
>* [AEM 6.3 檔案](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* 訂閱[Adobe優先產品更新](https://www.adobe.com/tw/subscription/priority-product-update.html)
